...
def execute(user_event):
    ...
    if len(event.parseString(user_event)) == 2:  # no argument
        cmd, dev = event.parseString(user_event)
        cmd_str, dev_str = ' '.join(cmd), ' '.join(dev)
        if 'open' in cmd_str or 'turn on' in cmd_str:
            open_actions[dev_str]()
        elif 'close' in cmd_str or 'turn off' in cmd_str:
            close_actions[dev_str]()
    elif len(event.parseString(user_event)) == 3:  # argument
        cmd, dev, arg = event.parseString(user_event)
        cmd_str = ' '.join(cmd)
        dev_str = ' '.join(dev)
        arg_str = ' '.join(arg)
        num_arg = 0
        try:
            ...
            print(f"expected number but got: '{arg_str.split()[0]}'")
...
if __name__ == '__main__':
    ...
    print()
    execute('increase -> boiler temperature -> -1 degrees')
